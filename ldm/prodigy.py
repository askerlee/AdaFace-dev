...
class ProdigyAdamW(torch.optim.Optimizer):
    ...
    def prodigy_step(self, closure=None):
        """Performs a single optimization step.
    ...
    def step(self, closure=None):
        # First prodigy update, then AdamW update.
        # Because prodigy is more aggressive, if doing it later, 
        # params have been updated by AdamW, and prodigy will 
        # go along the slightly wrong direction with large steps,
        # which may cause instability.
        # NOTE: Since in our setting, prodigy has no weight decay, p.grad is not changed. 
        # So we don't need to save it before adam update.
        # If you want to use prodigy with weight decay, 
        # you need to save p.grad before prodigy update, and restore p.grad 
        ...
        if self.weight_decay > 0:
            for group in self.param_groups:
                for p in group['params']:
                    ...
                    grad = p.grad.data
                    state = self.state[p]
                    grad.copy_(state['grad_backup'])
                    del state['grad_backup']
