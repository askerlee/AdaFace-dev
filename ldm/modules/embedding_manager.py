...
class AdaEmbedding(nn.Module):
    ...
    def __init__(self, num_layers=16, num_vectors_per_token=1, 
                 ...
                 # If subj_basis_generator is provided, then the basis vectors are generated by it 
                 # in a zero-shot fashion, instead of being learned from the data.
                 subj_basis_generator=None, 
                 device_type="cuda"):
        super().__init__()
        self.token_string = token_string
        ...
        self.subj_basis_generator = subj_basis_generator
        zero_shot_sig = "Zero-shot" if self.subj_basis_generator is not None else "Slow"
    ...
    def forward(self, layer_idx, layer_attn_components, time_emb, 
        ...
        with torch.autocast(device_type=self.device_type, enabled=True):
            ...
            if self.subj_basis_generator is not None:
                basis_vecs = self.subj_basis_generator()
            else:
                # self.N: number of pre_vecs.
                if self.N > 0:
...
class EmbeddingManager(nn.Module):
    ...
    def optimized_parameters(self):
        ...
        return ada_shared_params + private_params + slow_params_incl_prodigy + slow_params_excl_prodigy
