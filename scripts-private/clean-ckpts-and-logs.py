...
parser.add_argument("--mock", help="mock run", action="store_true")
...
def delete_all_but_largest_checkpoint(ckpt_folder_path):
    ...
    for filename in sorted(os.listdir(ckpt_folder_path)):
...
def clean_log_folders(root_log_folder, clean_ckpt_pat, skip_ckpt_pat=None, 
                      del_samples=False, is_mock_run=False):
    # Iterate through all subfolders (log folders)
    for subfolder in sorted(os.listdir(root_log_folder)):
...
if __name__ == "__main__":
    ...
    clean_log_folders(root_log_folder, clean_ckpt_pat=args.clean_ckpt_pat, skip_ckpt_pat=args.skip_ckpt_pat, 
                      del_samples=args.del_samples, is_mock_run=args.mock)
