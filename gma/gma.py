...
class Attention(nn.Module):
    def __init__(
        ...
        *,
        ...
        # These values don't matter, as in our case, q is always zero.
        # All schemes lead to the same uniform distribution of attention weights.
        if not hasattr(self.config, 'position_only'):
            self.config.position_only = True
        if not hasattr(self.config, 'position_and_content'):
            self.config.position_and_content = False
