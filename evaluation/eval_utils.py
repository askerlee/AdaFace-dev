...
def format_prompt_list(subject_string, z_prefix, z_suffix, background_string, 
                       class_name, class_long_name, 
                       broad_class, prompt_set_name='all', fp_trick_string=None):
    object_prompt_list = [
    # The space between "{0} {1}" is removed, so that prompts 
    # for ada/static-layerwise/ti could be generated by
    # providing an empty class_name. To generate prompts for DreamBooth, 
    # provide a class_name starting with a space.
    ...
    if class_name in subject_string:
        class_name = ""
    if class_long_name in subject_string:
        class_long_name = ""

    prompt_list            = [ prompt.format(z_prefix, subject_string,   z_suffix)           for prompt in orig_prompt_list ]
    orig_short_prompt_list = [ prompt.format(z_prefix, class_name,      background_string)  for prompt in orig_prompt_list ]
    orig_long_prompt_list  = [ prompt.format(z_prefix, class_long_name, background_string)  for prompt in orig_prompt_list ]
