...
```bash
python3 main.py --base configs/stable-diffusion/v1-finetune-ada.yaml -t --base_model_path models/stable-diffusion-v-1-5/v1-5-dste8-vae.ckpt --gpus 0,1 --data_roots /path/to/VGGface2_HQ_masks --mix_subj_data_roots /path/to/FFHQ_masks -n zero-ada --no-test --max_steps 120000 --subject_string z --background_string y --num_vectors_per_bg_token 4 --num_vectors_per_subj_token 16 --clip_last_layers_skip_weights 1 2 2 --randomize_clip_skip_weights --warmup_steps 600 --d_coef 1 --bs 4 --p_unet_distill_iter 1 --comp_distill_iter_gap 0
```
...
```bash
python3 main.py --base configs/stable-diffusion/v1-finetune-ada.yaml -t --base_model_path models/stable-diffusion-v-1-5/v1-5-dste8-vae.ckpt --gpus 0,1 --data_roots /path/to/VGGface2_HQ_masks --mix_subj_data_roots /path/to/FFHQ_masks -n zero3-ada --no-test --max_steps 60000 --subject_string z --background_string y --num_vectors_per_bg_token 4 --num_vectors_per_subj_token 16 --clip_last_layers_skip_weights 1 2 2 --randomize_clip_skip_weights --warmup_steps 1000 --d_coef 0.5 --bs 3 --p_unet_distill_iter 0.2 --comp_distill_iter_gap 4 --adaface_ckpt_paths logs/VGGface2_HQ_masks2024-04-29T18-19-49_zero3-ada/checkpoints/embeddings_gs-30000.pt --extend_prompt2token_proj_attention_multiplier 2 --load_meta_subj2person_type_cache_path /path/to/meta_subj2person_type.json --max_num_unet_distill_denoising_steps 5
```

Note that the `--comp_distill_iter_gap 4` option specifies that the compositionality distillation is performed every 4 iterations. The `--max_num_unet_distill_denoising_steps 5` option specifies that the multi-step denoising is performed at most 5 times. The `--p_unet_distill_iter 0.2` option specifies that the AdaFace distillation on an Arc2Face teacher model is performed with a probability of `0.2` (in contrast to `1` in the first stage). The `--extend_prompt2token_proj_attention_multiplier 2` option increases the number of K and V in attention layers by 2x in the prompt2token projection CLIP model.
