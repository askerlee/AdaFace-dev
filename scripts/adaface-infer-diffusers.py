...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt", type=str, default="a woman in superman costume")
    ...
    string_to_subj_basis_generator_dict = load_subj_basis_generators(args.embman_ckpt)
    ...
    placeholder_tokens = []
    for i in range(0, args.num_vectors):
        placeholder_tokens.append(f"{args.subject_string}_{i}")

    # Add z0, z1, z2, ..., z15.
    num_added_tokens = tokenizer.add_tokens(placeholder_tokens)
    if num_added_tokens != args.num_vectors:
        raise ValueError(
            f"The tokenizer already contains the token {args.subject_string}. Please pass a different"
    ...
    arc2face_text_encoder = arc2face_text_encoder.to('cuda')
    ...
    # Otherwise, faceid_embeds is a batch of num_images random embeddings, different from each other.
    ...
    faceid_embeds, id_prompt_emb \
        = get_arc2face_id_prompt_embs(face_app, tokenizer, arc2face_text_encoder,
                                      ...
                                      # including 4 template tokens and 16 ID tokens, and BOS and EOS tokens.
                                      input_max_length=22,
    ...
    with torch.no_grad():
        for i, token_id in enumerate(placeholder_token_ids):
            token_embeds[token_id] = adaface_subj_embs[0, i]

    comp_prompt = " ".join(placeholder_tokens) + " " + args.prompt
    ...
    save_images(images, subject_name, f"guide{args.guidance_scale}", noise_level)
