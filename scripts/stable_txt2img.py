...
def parse_args():
    ...
    parser.add_argument("--normalize_cross_attn", type=str2bool, nargs="?", const=True, default=False,
...
def main(opt):
    ...
    if opt.use_ldm_pipeline:
        ...
        config.model.params.normalize_cross_attn = opt.normalize_cross_attn
    ...
    if opt.diffusers:
        if opt.method == "adaface":
            ...
            pipeline = AdaFaceWrapper("text2img", opt.ckpt, opt.adaface_encoder_types, 
                                      ...
                                      normalize_cross_attn=opt.normalize_cross_attn,
                                      device=device)
