...
def parse_subject_file(subject_file_path, method):
    subjects, class_tokens, are_animals = None, None, None
            ...
            if re.search(r"^set -[la] (subjects|db_prompts|cls_tokens|are_animals)", line):
                ...
                if mat is not None:
                    ...
                    if var_name == "subjects":
                        ...
                        class_tokens = substrings
                    elif var_name == "are_animals":
                        are_animals = [ int(s) for s in substrings ]
                else:
                    breakpoint()

    if are_animals is None:
        # By default, all subjects are humans/animals, unless specified in the subject file.
        are_animals = [ 1 for _ in subjects ]

    if subjects is None or class_tokens is None:
        raise ValueError("subjects or db_prompts is None")
    
    return subjects, class_tokens, are_animals

def get_promt_list(subject_name, unique_token, class_token, is_animal):
    object_prompt_list = [
...
args = parse_args()
subjects, class_tokens, are_animals = parse_subject_file(args.subject_file, args.method)
if args.method == 'db':
...
for subject_name, class_token, is_animal in zip(subjects, class_tokens, are_animals):
    ...
    # E.g., get_promt_list(subject_name="cat2", placeholder="z", class_token="cat", is_animal=True)
    prompt_list, orig_prompt_list = get_promt_list(subject_name, args.placeholder, class_token, is_animal)
