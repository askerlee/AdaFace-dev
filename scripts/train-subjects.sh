...
argparse --ignore-unknown --min-args 1 --max-args 40 'gpu=' 'maxiter=' 'lr=' 'subjfile=' 'bb_type=' 'num_vectors_per_subj_token=' 'clip_last_layers_skip_weights=' 'use_conv_attn_kernel_size=' 'eval' -- $argv
or begin
    echo "Usage: $self [--gpu ID] [--maxiter M] [--lr LR] [--subjfile SUBJ] [--bb_type bb_type] [--num_vectors_per_subj_token K] [--clip_last_layers_skip_weights w1,w2,...] [--eval] [--use_conv_attn_kernel_size K] (ada|ti|db) [low-high] [EXTRA_ARGS]"
...
if set -q _flag_num_vectors_per_subj_token
    set EXTRA_TRAIN_ARGS0 $EXTRA_TRAIN_ARGS0 --num_vectors_per_subj_token $_flag_num_vectors_per_subj_token
    set EXTRA_EVAL_ARGS0 $EXTRA_EVAL_ARGS0   --num_vectors_per_subj_token $_flag_num_vectors_per_subj_token
end
...
for i in $indices
    ...
    set cls_delta_string  $cls_delta_strings[$i]
    set subj_init_word_weights  (string split " " $all_init_word_weights[$i])
    set cls_string $cls_delta_string
    ...
    if [ $method = 'ti' ]; or [ $method = 'ada' ]; or [ $method = 'static-layerwise' ]
        ...
        if set -q resume_from_ckpt; and test $resume_from_ckpt -eq 1
            ...
            set EXTRA_TRAIN_ARGS1 $EXTRA_TRAIN_ARGS1 --embedding_manager_ckpt $emb_man_ckpt --emb_reg_loss_scale 0.2
        end

        echo $subject: --cls_delta_string $cls_delta_string $EXTRA_TRAIN_ARGS1
        set fish_trace 1
        python3 main.py --base configs/stable-diffusion/v1-finetune-$method.yaml  -t --actual_resume $sd_ckpt --gpus $GPU, --data_roots $data_folder/$subject/ -n $subject-$method --no-test --max_steps $max_iters --subject_string "z" --cls_delta_string $cls_delta_string --subj_init_word_weights $subj_init_word_weights --broad_class $broad_class $EXTRA_TRAIN_ARGS1
        ...
        python3 main_db.py --base configs/stable-diffusion/v1-finetune-db.yaml -t --actual_resume $sd_ckpt --gpus $GPU, --reg_data_root regularization_images/(string replace -a " " "" $cls_delta_string) --data_roots $data_folder/$subject -n $subject-db --no-test --token "z" --class_word $cls_delta_string
