...
argparse --ignore-unknown --min-args 1 --max-args 20 'gpu=' 'maxiter=' 'lr=' 'subjfile=' 'bb_type=' 'num_vectors_per_token=' 'clip_last_layers_skip_weights=' 'cls_token_as_delta' 'use_conv_attn' 'eval' 'use_deep_neg_prompt' 'normalize_subj_attn' 'learnable_deep_neg_token=' -- $argv
or begin
...
set -q _flag_use_deep_neg_prompt; and set EXTRA_EVAL_ARGS0 $EXTRA_EVAL_ARGS0 --use_deep_neg_prompt

if set -q _flag_learnable_deep_neg_token
    set EXTRA_TRAIN_ARGS0 $EXTRA_TRAIN_ARGS0 --learnable_deep_neg_token $_flag_learnable_deep_neg_token
