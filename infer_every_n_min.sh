...
RANGE=$2
SIG=$3
fish -c "cd ~/adaprompt; python3 -m scripts.gen_prompts_and_eval --out_dir_tmpl samples --subjfile ~/adaprompt/evaluation/info-subjects-ood.sh --adaface_ckpt_paths (ls -rt ~/adaprompt/logs/VGGface2_HQ_masks2024-*$SIG*_zero3-ada/checkpoints/embedding*|tail -1) --z_suffix_type class_name --method adaface --sep_log  --fp_trick_str portrait --gpus 0-1 --scale 6 --range $RANGE"
